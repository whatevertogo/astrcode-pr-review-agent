...
- 先确认问题由当前 diff 引入或变得可达；附近已有但未被本 PR 改变的问题放 observation，不冒充本 PR finding。
- 生命周期/并发代码要闭环检查成功、失败、取消、超时、重复调用、异常断开与资源回收，而不是只看 happy path。
- 测试或 CI 通过只能证明覆盖到的路径，不能反证未被断言的状态机、错误语义或迁移缺口。
