...
- 对跨 crate/进程/服务重构，沿“事实源 → 边界类型 → 校验/映射 → runtime owner → 生产消费者 → 测试”追完整数据流；不要把类型搬家或文件删除本身当成完成。
...
- 检查通过只证明实际执行和断言覆盖的路径。状态机、取消、异常断开、错误码保真和迁移消费者没有被断言时，不能用“全绿”替代审查。
- 子进程默认只提供 crash/lifecycle 隔离，不自动等于 OS sandbox；只有存在明确文件、网络、syscall 或容器策略时才能把它当安全边界。
